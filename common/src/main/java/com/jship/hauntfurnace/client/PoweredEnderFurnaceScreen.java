...
public class PoweredEnderFurnaceScreen extends AbstractContainerScreen<PoweredEnderFurnaceMenu> implements RecipeUpdateListener {
    public final AbstractFurnaceRecipeBookComponent recipeBookComponent;
    private boolean widthTooNarrow;
    public static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(HauntFurnace.MOD_ID, "textures/gui/container/powered_ender_furnace.png");
    public static final ResourceLocation POWERED_PROGRESS_TEXTURE = ResourceLocation.fromNamespaceAndPath(HauntFurnace.MOD_ID, "container/ender_furnace/powered_progress");
    ...
    public PoweredEnderFurnaceScreen(PoweredEnderFurnaceMenu menu, Inventory inventory, Component title) {
        super(menu, inventory, title);
        this.recipeBookComponent = new EnderFurnaceRecipeBookComponent();
    }
        ...
        if (x >= this.leftPos + 9 && x <= this.leftPos + 22 && y >= this.topPos + 26 && y <= this.topPos + 59) {
            guiGraphics.renderTooltip(this.font,
                    Component.translatable("gui.screen.hauntfurnace.energy",
                            ((PoweredEnderFurnaceMenu) this.menu).energy(),
                            PoweredEnderFurnaceBlockEntity.ENERGY_CAPACITY),
                    x, y);
        }
    }
    ...
    protected void renderBg(GuiGraphics guiGraphics, float delta, int mouseX, int mouseY) {
        ...
        int cookProgress = ((PoweredEnderFurnaceMenu) this.menu).getCookProgress();
