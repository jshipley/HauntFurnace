...
import com.jship.hauntfurnace.block.entity.AbstractPoweredFurnaceBlockEntity;
...
public abstract class AbstractPoweredFurnaceMenu<T extends AbstractCookingRecipe> extends RecipeBookMenu {
    ...
    public AbstractPoweredFurnaceMenu(MenuType<?> menuType, int containerId, Inventory playerInventory, ResourceKey<RecipePropertySet> recipePropertySet) {
        this(
            menuType,
            containerId,
            playerInventory,
            new SimpleContainer(SLOT_COUNT),
            new SimpleContainerData(AbstractPoweredFurnaceBlockEntity.NUM_DATA_VALUES),
            recipePropertySet
        );
    }

    public AbstractPoweredFurnaceMenu(
        MenuType<?> menuType,
        int containerId,
        Inventory inventory,
        Container container,
        ...
        checkContainerDataCount(data, AbstractPoweredFurnaceBlockEntity.NUM_DATA_VALUES);
        this.container = container;
        this.data = data;
        this.level = inventory.player.level();
        this.acceptedInputs = this.level.recipeAccess().propertySet(recipePropertySet);
    ...
    public boolean recipeMatches(RecipeHolder<T> recipe) {
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (slot2 != null && slot2.hasItem()) {
            ...
            if (index == RESULT_SLOT) {
                ...
                } else if (index >= USE_ROW_SLOT_START && index < USE_ROW_SLOT_END && !this.moveItemStackTo(itemStack2, INV_SLOT_START, INV_SLOT_END, false)) {
                    return ItemStack.EMPTY;
                }
    ...
    public int getCookingProgress() {
        int cookingTimer = this.data.get(AbstractPoweredFurnaceBlockEntity.DATA_COOKING_PROGRESS);
        int cookingTotalTime = this.data.get(AbstractPoweredFurnaceBlockEntity.DATA_COOKING_TOTAL_TIME);
        return cookingTotalTime != 0 && cookingTimer != 0 ? (cookingTimer * 24) / cookingTotalTime : 0;
    }

    public boolean isLit() {
        return this.data.get(AbstractPoweredFurnaceBlockEntity.DATA_LIT_TIME) > 0;
    }

    public int energyLevel() {
        // the power meter is 32 pixels tall
        return ((32 * this.data.get(AbstractPoweredFurnaceBlockEntity.DATA_ENERGY_STORAGE)) / AbstractPoweredFurnaceBlockEntity.ENERGY_CAPACITY);
    }

    public int energy() {
        return this.data.get(AbstractPoweredFurnaceBlockEntity.DATA_ENERGY_STORAGE);
    }
    ...
    private class PHCraftingMenuAccess implements CraftingMenuAccess<T> {

        @Override
        public void clearCraftingContent() {
            AbstractPoweredFurnaceMenu.this.clearCraftingContent();
        }

        @Override
        public void fillCraftSlotsStackedContents(StackedItemContents stackedItemContents) {
            AbstractPoweredFurnaceMenu.this.fillCraftSlotsStackedContents(stackedItemContents);
        }

        @Override
        public boolean recipeMatches(RecipeHolder<T> recipe) {
            return AbstractPoweredFurnaceMenu.this.recipeMatches(recipe);
        }
    }
}
