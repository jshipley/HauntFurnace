...
public class PoweredEnderFurnaceMenu extends RecipeBookMenu {
    ...
    public PoweredEnderFurnaceMenu(int containerId, Inventory playerInventory) {
        this(containerId, playerInventory, new SimpleContainer(SLOT_COUNT), new SimpleContainerData(PoweredEnderFurnaceBlockEntity.NUM_DATA_VALUES));
    }
    ...
    public boolean recipeMatches(RecipeHolder<CorruptingRecipe> recipe) {
    ...
    public int getHauntProgress() {
        int cookingTimer = this.data.get(PoweredEnderFurnaceBlockEntity.DATA_COOKING_PROGRESS);
        int cookingTotalTime = this.data.get(PoweredEnderFurnaceBlockEntity.DATA_COOKING_TOTAL_TIME);
        return cookingTotalTime != 0 && cookingTimer != 0 ? (cookingTimer * 24) / cookingTotalTime : 0;
    }

    public boolean isLit() {
        return this.data.get(PoweredEnderFurnaceBlockEntity.DATA_LIT_TIME) > 0;
    }

    public int energyLevel() {
        // the power meter is 32 pixels tall
        return ((32 * this.data.get(PoweredEnderFurnaceBlockEntity.DATA_ENERGY_STORAGE)) / PoweredEnderFurnaceBlockEntity.ENERGY_CAPACITY);
    }

    public int energy() {
        return this.data.get(PoweredEnderFurnaceBlockEntity.DATA_ENERGY_STORAGE);
    }
    ...
    public PostPlaceAction handlePlacement(boolean useMaxItems, boolean isCreative, RecipeHolder<?> recipe, ServerLevel level, Inventory playerInventory) {
        ...
        return ServerPlaceRecipe.placeRecipe(new PHCraftingMenuAccess(), this.getGridWidth(), this.getGridHeight(), List.of(this.getSlot(INGREDIENT_SLOT)), list, playerInventory, (RecipeHolder<CorruptingRecipe>) recipe, useMaxItems, isCreative);
    }

    private class PHCraftingMenuAccess implements CraftingMenuAccess<CorruptingRecipe> {

        @Override
        public void clearCraftingContent() {
            PoweredEnderFurnaceMenu.this.clearCraftingContent();
        }

        @Override
        public void fillCraftSlotsStackedContents(StackedItemContents stackedItemContents) {
            PoweredEnderFurnaceMenu.this.fillCraftSlotsStackedContents(stackedItemContents);
        }

        @Override
        public boolean recipeMatches(RecipeHolder<CorruptingRecipe> recipe) {
            return PoweredEnderFurnaceMenu.this.recipeMatches(recipe);
        }
    }
}
