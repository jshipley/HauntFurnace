...
public class PoweredEnderFurnaceMenu extends RecipeBookMenu<SingleRecipeInput, AbstractCookingRecipe> {
    ...
    public PoweredEnderFurnaceMenu(int syncId, Inventory inventory) {
        this(syncId, inventory, new SimpleContainer(CRAFT_SLOTS), new SimpleContainerData(PoweredEnderFurnaceBlockEntity.PROPERTY_COUNT));
    }

    public PoweredEnderFurnaceMenu(int syncId, Inventory inventory, Container container, ContainerData dataAccess) {
        super(HauntFurnace.POWERED_ENDER_FURNACE_MENU.get(), syncId);
        this.recipeBookType = RecipeBookType.FURNACE;
        checkContainerSize(container, CRAFT_SLOTS);
        checkContainerDataCount(dataAccess, PoweredEnderFurnaceBlockEntity.PROPERTY_COUNT);
    ...
    protected boolean isHauntable(ItemStack itemStack) {
        return this.level.getRecipeManager().getRecipeFor(HauntFurnace.CORRUPTING_RECIPE.get(), new SingleRecipeInput(itemStack), this.level).isPresent();
    }

    public int getCookProgress() {
        int i = this.dataAccess.get(PoweredEnderFurnaceBlockEntity.COOK_TIME_PROPERTY_INDEX);
        int j = this.dataAccess.get(PoweredEnderFurnaceBlockEntity.COOK_TIME_TOTAL_PROPERTY_INDEX);
        return j != 0 && i != 0 ? i * 24 / j : 0;
    }

    public boolean isBurning() {
        return this.dataAccess.get(PoweredEnderFurnaceBlockEntity.COOK_TIME_PROPERTY_INDEX) > 0;
    }

    public int energyLevel() {
        // the power meter is 32 pixels tall
        return (32 * this.dataAccess.get(PoweredEnderFurnaceBlockEntity.ENERGY_STORAGE_PROPERTY_INDEX)) / PoweredEnderFurnaceBlockEntity.ENERGY_CAPACITY;
    }

    public int energy() {
       return this.dataAccess.get(PoweredEnderFurnaceBlockEntity.ENERGY_STORAGE_PROPERTY_INDEX);
    }
