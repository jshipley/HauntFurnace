...
public class CorruptingRecipeClientCategory implements DisplayCategory<CorruptingRecipeClientDisplay> {
    ...
    public CorruptingRecipeClientCategory(
        CategoryIdentifier<CorruptingRecipeClientDisplay> identifier,
        EntryStack<?> logo,
        String categoryName,
        double defaultCookingTime
    ) {
    ...
    public List<Widget> setupDisplay(CorruptingRecipeClientDisplay display, Rectangle bounds) {
        ...
        widgets.add(
            new BurningAltFireWidget(new Rectangle(new Point(startPoint.x + 1, startPoint.y + 20), new Dimension(14, 14)), 2).animationDurationMS(10000)
        );
            ...
            widgets.add(
                Widgets.createLabel(
                    ...
                    Component.translatable(
                        ...
                        df.format(display.xp().getAsDouble()),
                        df.format(display.cookTime().getAsDouble() / 20d)
                    )
                )
                    .noShadow()
                    .rightAligned()
        ...
        widgets.add(Widgets.createArrow(new Point(startPoint.x + 24, startPoint.y + 8)).animationDurationTicks(display.cookTime().orElse(defaultCookingTime)));
        widgets.add(
